...
export default function RegistrationScreen({ route, navigation }) {
  ...
  const onEmailLoseFocus = () => {
    ...
    if (email.length === 0) {
      ...
      email.substring(email.indexOf("@") + 1, email.length).indexOf(".") < 0
    ) {
      setIsEmailAdequate(false);
    } else if (
      temp.length > 0 &&
      temp[0].emailHost != email.substring(email.indexOf("@") + 1, email.length)
    ) {
      setIsEmailAdequate(false);
    } else {
      setIsEmailAdequate(true);
  ...
  const handleRegistration = () => {
    if (
      ...
      !agreeAll &&
      (!serviceAgree || !communityAgree || !personalAgree)
    ) {
    ...
    authService
      ...
      .then(async (userCredential) => {
        ...
        if (profileSelected) {
          ...
          const snapshot = await storageService
            .ref("profile/" + user.uid)
            .put(blob);
          profileUrl = await snapshot.ref.getDownloadURL();
        }
        const uploadProfile = await dbService
          .collection("profile")
          .doc(country)
        ...
        const uploadProfileRef = await dbService
          .collection("profileRef")
          .doc(user.uid)
          .set({
            country: country,
        ...
        const initialSetup = await dbService
          .collection("profile")
          .doc(country)
          .collection(university)
          .doc(user.uid)
        ...
        await authService.currentUser
          ...
          .then(() => {
            ...
            Alert.alert(
              "회원 가입 성공!",
              "A verification email has been sent to registered email address. Please click on the link in the email to activate your account. \n\n 계정 확인 메일이 입력하신 메일주소로 발송되었습니다. 이메일에 있는 링크를 눌러 계정을 활성화해주세요."
            );
          ...
          .catch((err) => {
            ...
            Alert.alert(
              "회원가입 실패",
              "알 수 없는 오류로 회원가입을 처리할 수 없습니다. 잠시 후 다시 시도해주세요."
            );
            return;
          });
      })
          ...
          <ScrollView style={styles.container}>
            ...
            <View>
              ...
              <DropDownPicker
                ...
                textStyle={{ fontFamily: "Candal", fontSize: 10 }}
              ...
              />
              <Text style={styles.findYourUniversity}>
              ...
              <DropDownPicker
                ...
                containerStyle={styles.univContainerStyle}
                textStyle={{ fontFamily: "Candal", fontSize: 10 }}
                open={universityPickerOpen}
                value={university}
                items={displayedUniversity}
                onOpen={onUnivOpen}
                setOpen={setUniversityPickerOpen}
                setValue={onSelectUniversity}
                setItems={changeUniversityData}
              />
              ...
              {isNameAdequate ? (
                ...
                <Text style={styles.alert}>
                  이름은 특수기호를 포함할 수 없습니다.
                </Text>
              )}
              <TextInput
                onBlur={onNicknameLoseFocus}
              ...
              {isNicknameAdequate ? (
                isNicknameUnique ? (
                  ...
                  <Text style={styles.alert}>닉네임이 이미 존재합니다.</Text>
                )
              ) : (
                <Text style={styles.alert}>
                  닉네임은 특수기호를 포함할 수 없습니다.
                </Text>
              )}
              <TextInput
              ...
              {isEmailAdequate ? (
                ...
                <Text style={styles.alert}>이메일 형식이 잘못되었습니다.</Text>
              )}
              <TextInput
                onBlur={onPasswordLoseFocus}
              ...
              <TextInput
                onBlur={onCheckPwLoseFocus}
                style={styles.checkPw}
                onChangeText={changeCheckPw}
              ...
              {isCheckPwAdequate ? (
                ...
                <Text style={styles.alert}>비밀번호와 일치하지 않습니다.</Text>
              )}
            </View>
            ...
            <View style={{ display: "flex", flexDirection: "column" }}>
              ...
              >
                <CheckBox
                  isChecked={agreeAll}
                  onClick={() => selectAgreeAll(!agreeAll)}
              ...
              >
                <CheckBox
                  ...
                  onClick={() => selectService(!serviceAgree)}
                  style={styles.serviceTerm}
                  checkedCheckBoxColor={"#FFDE00"}
                  uncheckedCheckBoxColor={"#FFDE00"}
                />
                <Text style={styles.serviceTermText}>
                  서비스 이용약관 동의 (필수)
                </Text>
              </View>
              <View
              ...
              >
                <CheckBox
                  isChecked={personalAgree}
                  onClick={() => selectPersonal(!personalAgree)}
                  style={styles.personalTerm}
                  checkedCheckBoxColor={"#FFDE00"}
                  uncheckedCheckBoxColor={"#FFDE00"}
                />
                <Text style={styles.personalTermText}>
                  개인정보처리방침 동의 (필수)
                </Text>
              </View>
              <View
              ...
              >
                <CheckBox
                  isChecked={communityAgree}
                  onClick={() => selectCommunity(!communityAgree)}
                  style={styles.communityTerm}
                  checkedCheckBoxColor={"#FFDE00"}
                  uncheckedCheckBoxColor={"#FFDE00"}
                />
                <Text style={styles.communityTermText}>
                  커뮤니티 이용규칙 확인 (필수)
                </Text>
              </View>
              <View
              ...
              >
                <CheckBox
                  isChecked={adAgree}
                  onClick={() => selectAd(!adAgree)}
                  style={styles.adTerm}
                  checkedCheckBoxColor={"#FFDE00"}
                  uncheckedCheckBoxColor={"#FFDE00"}
                />
                <Text style={styles.adTermText}>
                  광고성 정보 수신 동의 (선택)
                </Text>
              </View>
            </View>
            ...
            >
              <Text style={{ fontFamily: "Candal", color: "white" }}>
                Registration
              </Text>
            </TouchableOpacity>
          </ScrollView>
